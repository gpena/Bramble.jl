...
# at any physical point, not only at its own mesh's points. `interpolate!`/`interpolate` are
# exactly `Rₕ!`/`Rₕ` applied to `x -> interpolate_at(src, x)`, so the checks below are about
...
@testset "Interpolation (point 25)" begin
    ...
    @testset "interpolate/interpolate! move a grid function across DIFFERENT meshes" begin
        ...
        dest2 = similar(dest)
        returned = interpolate!(dest2, src)
        @test returned === dest2
        @test values(dest2) ≈ values(dest)
    end

    @testset "interpolation_matrix agrees with interpolate, in 1D and 2D" begin
        # P * values(src) is exactly the same computation interpolate performs pointwise —
        ...
        @test P1 * values(src1) ≈ values(interpolate(W1dest, src1))
        ...
        @test P2 * values(src2) ≈ values(interpolate(W2dest, src2))
    ...
    @testset "composes with the numeric difference/average operators" begin
        # once interpolate returns an ordinary VectorElement, every existing numeric
