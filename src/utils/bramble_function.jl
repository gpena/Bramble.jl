...
@inline has_time(::Type{<:BrambleFunction{
    ArgsType, hastime}}) where {ArgsType, hastime} = hastime
...
@inline (f::BrambleFunction{AT, false})(coords::Tuple{Number}) where {AT <:
                                                                      Number} = f.wrapped(convert(
    AT, coords[1]))
@inline (f::BrambleFunction{AT, false})(coords::SVector{1}) where {AT <:
                                                                   Number} = f.wrapped(convert(AT, coords[1]))
@inline (f::BrambleFunction{AT, false})(coords::AbstractVector) where {AT <:
                                                                       Number} = f.wrapped(convert(
    AT, coords[1]))
...
@inline (f::BrambleFunction{AT, false})(coords::Tuple) where {AT <:
                                                              Tuple} = f.wrapped(convert(AT, coords))
@inline (f::BrambleFunction{AT, false})(coords::SVector) where {AT <:
                                                                Tuple} = f.wrapped(convert(AT, Tuple(coords)))
@inline (f::BrambleFunction{AT, false})(coords::AbstractVector) where {AT <:
                                                                       Tuple} = f.wrapped(convert(
    AT, Tuple(coords)))
@inline (f::BrambleFunction{AT, false})(coords::Number...) where {AT <:
                                                                  Tuple} = f.wrapped(convert(AT, coords))
...
@inline embed_function(space_domain, time_domain::CartesianProduct{1}, func) = _embed_withtime(
    space_domain, time_domain, func)
...
function _embed_withtime(space_domain, time_domain::CartesianProduct{1}, f; FinalCoType = eltype(time_domain))
    ...
    return bramble_function_with_domain(
        _f, time_domain, true, CoType; domain = time_domain)::BFType
end
