...
"""
@inline function form(Wₕ::SpaceType, _f::F; strategy = DefaultAssembly(), verbose::Bool = false) where F
	available_strategy = []
...
@inline _assemble(l::LinearForm, ::DefaultAssembly) = l.form_expr(elements(testspace(l)))
@inline _assemble(l::LinearForm, ::OperatorsAssembly) = l.form_expr(IdentityOperator(testspace(l)))
...
"""
@inline assemble!(x::AbstractVector, l::LinearForm) = _assemble!(x, l, l.strategy)

@inline _assemble!(x::AbstractVector, l::LinearForm, ::DefaultAssembly) = x .= l.form_expr(elements(testspace(l)))
@inline _assemble!(x::AbstractVector, l::LinearForm, ::OperatorsAssembly) = x .= l.form_expr(IdentityOperator(testspace(l)))
@inline _assemble!(x::AbstractVector, l::LinearForm, ::InPlaceAssembly) = l.form_expr(x, IdentityOperator(testspace(l)))
...
"""
@inline assemble!(x::VectorElement, l::LinearForm) = (assemble!(x.values, l))
