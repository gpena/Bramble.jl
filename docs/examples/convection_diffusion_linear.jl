...
function solve_convection_diffusion(problem::LinearConvectionDiffusionProblem,
        nPoints::NTuple{D, Int}, unif::NTuple{D, Bool}) where {D}
    Ω = domain(problem)
...
function test_conv_diff(problem::LinearConvectionDiffusionProblem, nTests,
        npoints_generator, unif::NTuple{D, Bool}) where {D}
    error = zeros(nTests)
    hmax = zeros(nTests)
