...
"""
@inline Base.similar(Uₕ::MatrixElement{S, T, MT}) where {S, T, MT} = MatrixElement{S, T, MT}(
    similar(Uₕ.data), space(Uₕ))

@forward MatrixElement.data (Base.size, Base.length, Base.ndims, Base.firstindex,
    Base.lastindex, Base.iterate, Base.axes, Bramble.show)
...
@inline Base.@propagate_inbounds getindex(Uₕ::MatrixElement, I::Vararg{
    Int, N}) where {N} = getindex(Uₕ.data, I...)
@inline Base.@propagate_inbounds getindex(Uₕ::MatrixElement, I::NTuple{
    N, Int}) where {N} = getindex(Uₕ.data, I...)

@inline Base.@propagate_inbounds setindex!(Uₕ::MatrixElement, v, i::Int) = (
    setindex!(Uₕ.data, v, i); return)
@inline Base.@propagate_inbounds setindex!(Uₕ::MatrixElement, v, I::Vararg{
    Int, N}) where {N} = (setindex!(Uₕ.data, v, I...); return)
@inline Base.@propagate_inbounds setindex!(Uₕ::MatrixElement, v, I::NTuple{
    N, Int}) where {N} = (setindex!(Uₕ.data, v, I...); return)
...
@inline Base.:*(uₕ::VectorElement, Vₕ::NTuple{D, MatrixElement}) where {D} = ntuple(
    i -> uₕ *
         Vₕ[i], Val(D))
...
@inline ⋅(Uₕ::NTuple{D, VectorElement}, Vₕ::NTuple{D, MatrixElement}) where {D} = ntuple(
    i -> Uₕ[i] *
         Vₕ[i], D)

for op in (:+, :-, :*)
    dict = Dict(:+ => ("adding", "to", true), :- => ("subtracting", "to", false),
        :* => ("multiplying", "by", true))
    w1, w2, w3 = dict[op]
    ...
    @eval begin
        @doc $docstr @inline (Base.$op)(Uₕ::MatrixElement, Vₕ::MatrixElement) = elements(
            space(Uₕ), (Base.$op)(Uₕ.data, Vₕ.data))
    end
end
...
for op in (:+, :-, :*, :/)
    ...
    docstr1 = "	" * string(op) * "(α::Number, Uₕ::MatrixElement)" * same_text * " `α` " *
              string(op) * " `Uₕ`."
    docstr2 = "	" * string(op) * "(Uₕ::MatrixElement, α::Number)" * same_text * " `Uₕ` " *
              string(op) * " `α`."
