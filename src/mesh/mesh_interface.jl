...
This file defines the abstract interface that all mesh types must implement.
It provides the base type `AbstractMeshType` and declares the required interface methods.

## Interface Requirements

Any concrete mesh type (Mesh1D, MeshnD) must implement:

### Required Fields
- `set`: The geometric set
- `indices`: CartesianIndices of mesh points
- `backend`: Linear algebra backend
- `markers`: DomainMarkers for boundary/subdomain identification

### Required Methods
- `eltype`, `dim`, `topo_dim`
- `points`, `point`, `points_iterator`
- `half_points`, `half_point`, `half_points_iterator`
- `spacing`, `forward_spacing`, `half_spacing` (+ iterators)
- `npoints`, `hₘₐₓ`, `cell_measure`
- `iterative_refinement!`, `change_points!`

## Usage

Users typically don't interact with this interface directly. Instead, use the
`mesh` constructor function which automatically creates the appropriate concrete type.
"""
...
Any concrete subtype of [AbstractMeshType](@ref) must have a field called
`indices` of type `CartesianIndices`.
"""
...
This function is a required part of the [AbstractMeshType](@ref) interface.
Any concrete subtype of [AbstractMeshType](@ref) must have a field called
`markers` of type [DomainMarkers](@ref).
"""
...
Checks if the `CartesianIndex` `idx` lies on the boundary of the mesh `Ωₕ`.

This delegates to the implementation for `CartesianIndices`.
"""
...
This function is a required part of the [AbstractMeshType](@ref) interface.
Can be applied to the type of an [AbstractMeshType](@ref).
"""
...
The topological dimension counts the number of dimensions with more than one point,
effectively identifying collapsed dimensions in the mesh.
"""
...
Returns an iterator for the [forward_spacing](@ref), for each submesh.
"""
...
```math
h_{x,N+1/2} \\vcentcolon = \\frac{h_{N}}{2}, \\, h_{x,1/2} \\vcentcolon = \\frac{h_1}{2}
```
...
If `Tuple` is passed as the second argument, it returns a tuple with the number of
points of each submesh composing `Ωₕ`.
"""
...
(can be a `CartesianIndex` or a `Tuple`):
...
Refines the given mesh `Ωₕ` by halving each existing cell (in every direction).

If an object of type [DomainMarkers](@ref) is passed as an argument, it also updates
the markers accordingly after the refinement.
"""
...
Changes the coordinates of the internal points of the mesh `Ωₕ` to the new coordinates
specified in `pts`.

This function assumes the points in `pts` are ordered and that the first and last of them
coincide with the bounds of the domain. If the domain `Ω` is passed as an argument, the
markers of the mesh are also recalculated after this change.
"""
