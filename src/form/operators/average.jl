...
function vectorial_avg_backward(op::LazyOp{D}) where {D}
    return ntuple(dim -> BackwardAverage{D,dim,typeof(op)}(op), Val(D))
end
...
function vectorial_avg_forward(op::LazyOp{D}) where {D}
    return ntuple(dim -> ForwardAverage{D,dim,typeof(op)}(op), Val(D))
end
...
function resolve_ast(op::BackwardAverage{D,Dim}) where {D,Dim}
    return BackwardAverage{D,Dim,typeof(resolve_ast(op.inner_op))}(resolve_ast(op.inner_op))
end
function resolve_ast(op::ForwardAverage{D,Dim}) where {D,Dim}
    return ForwardAverage{D,Dim,typeof(resolve_ast(op.inner_op))}(resolve_ast(op.inner_op))
end
