...
function D₋ₓ(U::VectorElement)
	h = Base.Fix1(spacing, mesh(space(U))(1))
	dims = npoints(mesh(space(U)), Tuple)
	V = similar(U)
...
function D₋ᵧ(U::VectorElement)
	h = Base.Fix1(spacing, mesh(space(U))(2))
	dims = npoints(mesh(space(U)), Tuple)
	V = similar(U)
...
function D₋₂(U::VectorElement)
	D = dim(mesh(space(U)))
	h = Base.Fix1(spacing, mesh(space(U))(3))
	dims = npoints(mesh(space(U)), Tuple)
	V = similar(U)
...
@inline shift(M::MeshType, ::Val{i}) where i = _Eye(eltype(typeof(M)), npoints(M, Tuple)[1], i)
@inline shift(M::MeshType, ::Val{0}) = Eye(npoints(M, Tuple)[1])
...
**Output:**
  ...
  - a sparse matrix
"""
@inline shiftₓ(M::MeshType, ::Val{1}, ::Val{0}) = _Eye(eltype(M), npoints(M, Tuple)[1], 0)
...
**Output:**
  ...
  - a sparse matrix
"""
@inline shiftₓ(M::MeshType, ::Val{D}, ::Val{0}) where D = Eye(npoints(M))
...
@inline shiftₓ(M::MeshType, ::Val{D}, ::Val{i}) where {D,i} = Eye(npoints(M(D))) ⊗ shiftₓ(M, Val(D-1), Val(i))

@inline shiftᵧ(M::MeshType, ::Val{2}, ::Val{0}) = Eye(npoints(M))
@inline shiftᵧ(M::MeshType, ::Val{3}, ::Val{0}) = Eye(npoints(M))
@inline shiftᵧ(M::MeshType, ::Val{1}, ::Val{i}) where i = shift(M(2), Val(i))
@inline shiftᵧ(M::MeshType, ::Val{2}, ::Val{i}) where i = shiftᵧ(M, Val(1), Val(i)) ⊗ Eye(npoints(M(1)))
@inline shiftᵧ(M::MeshType, ::Val{3}, ::Val{i}) where i = Eye(npoints(M(3))) ⊗ shiftᵧ(M, Val(2), Val(i))

@inline shift₂(M::MeshType, ::Val{3}, ::Val{0}) = Eye(npoints(M))
@inline shift₂(M::MeshType, ::Val{3}, ::Val{i}) where i = shift(M(3), Val(i)) ⊗ Eye(prod(npoints(M, Tuple)[1:2]))
...
@inline diff₋ₓ(u::MatrixElement) = elements(space(u), diff₋ₓ(mesh(space(u)))*u.values)
@inline diff₋ᵧ(u::MatrixElement) = elements(space(u), diff₋ᵧ(mesh(space(u)))*u.values)
@inline diff₋₂(u::MatrixElement) = elements(space(u), diff₋₂(mesh(space(u)))*u.values)
...
function invert_spacing!(v, M::MeshType, component::Int = 1)
	for (i, h) in zip(eachindex(v), spacing(M(component), Iterator))
		v[i] = inv(h)
	end
end
...
**Inputs:**
  ...
  - `M`: a mesh
"""
@inline weights_D₋ₓ!(v, M::MeshType, ::Val{1}) = invert_spacing!(v, M)
...
@inline function weights_D₋ₓ!(v, M::MeshType, ::Val{D}) where D
	first_dims = prod(npoints(M, Tuple)[1:(D - 1)])
	weights_D₋ₓ!(view(v, 1:first_dims), M, Val(D - 1))

	repeat_across_dims1!(v, npoints(M(D)), first_dims)
end
...
@inline function weights_D₋ᵧ!(v, M::MeshType, ::Val{3})
	...
	repeat_across_dims1!(v, npoints(M(3)), first_dims)
end
...
@inline function create_backward_diff_matrices(S::SpaceType; diagonal = _create_diagonal(S))
	return ntuple(i -> _create_backward_diff_matrix(S, Val(i), diagonal = diagonal), dim(mesh(S)))
end
...
∇ₕ(u::VectorElement) = ntuple(i -> D₋ᵢ(u, Val(i)), dim(mesh(space(u))))
∇ₕ(u::MatrixElement) = ntuple(i -> D₋ᵢ(space(u), Val(i))*u, dim(mesh(space(u))))