...
@testset "MeshnD Tests" begin
    ...
    @testset "Helper Functions (D>1)" begin
        @testset "boundary_symbol_to_cartesian (D=2)" begin
        ...
        @testset "boundary_symbol_to_cartesian (D=3)" begin
    ...
    @testset "Dimension D=2" begin
        ...
        @testset "Index Subsets (D=2)" begin
            ...
            bnd_indices = collect(boundary_indices(Ωₕ_2d_unif))
            int_indices = collect(interior_indices(Ωₕ_2d_unif))

            @test length(bnd_indices) == 2*4 + 2*5 - 4 == 14 # Perimeter count
            @test CartesianIndex(1, 1) in bnd_indices
            @test CartesianIndex(4, 5) in bnd_indices
            @test CartesianIndex(1, 3) in bnd_indices
            @test CartesianIndex(3, 1) in bnd_indices
            @test CartesianIndex(4, 2) in bnd_indices
            @test CartesianIndex(2, 5) in bnd_indices
            @test !(CartesianIndex(2, 2) in bnd_indices)
            @test !(CartesianIndex(3, 4) in bnd_indices)
    ...
    @testset "Dimension D=3" begin
        ...
        @testset "Index Subsets (D=3)" begin
            ...
            bnd_indices_lg = collect(boundary_indices(Ωₕ_3d_larger))
            int_indices_lg = collect(interior_indices(Ωₕ_3d_larger))

            total_pts = prod(npts_3d_larger) # 4*5*4 = 80
            interior_pts = (4-2)*(5-2)*(4-2) # 2*3*2 = 12
            boundary_pts = total_pts - interior_pts # 80 - 12 = 68
            @test length(bnd_indices_lg) == boundary_pts
            @test CartesianIndex(1, 1, 1) in bnd_indices_lg
            @test CartesianIndex(4, 5, 4) in bnd_indices_lg
            @test CartesianIndex(2, 2, 1) in bnd_indices_lg # On face z=1
            @test CartesianIndex(2, 1, 2) in bnd_indices_lg # On face y=1
            @test CartesianIndex(1, 3, 3) in bnd_indices_lg # On face x=1
            @test !(CartesianIndex(2, 2, 2) in bnd_indices_lg)
            @test !(CartesianIndex(3, 4, 3) in bnd_indices_lg)

            @test interior_indices(Ωₕ_3d_larger) == CartesianIndices((2:3, 2:4, 2:3))
            @test length(int_indices_lg) == interior_pts
