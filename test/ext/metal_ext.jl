...
if !Metal.functional() || !_run_gpu_tests()
    ...
    @testset "#323 fdm_solve" begin
        ...
        for (dom, dims) in ((I × I, (31, 17)), (I × I × I, (9, 8, 7)))
            Ωd = mesh(domain(dom), dims, ntuple(_ -> false, length(dims)); backend = metal_backend())
            Wd = gridspace(Ωd)
            Wh = gridspace(Bramble._host_mirror_mesh(mesh(Wd)))
            ad = form(Wd, Wd, f)
            ah = form(Wh, Wh, f)
            n = prod(dims)

            Fh = rand(Float32, n)
            xd = fdm_solve(ad, MtlArray(Fh))
            @test xd isa MtlArray
            @test isapprox(Array(xd), fdm_solve(ah, Fh); rtol = 1.0f-5)

            Fb = zeros(Float32, dims)
            Fb[map(m -> 2:(m - 1), dims)...] .= rand(Float32, map(m -> m - 2, dims))
            Fb = vec(Fb)
            xb = fdm_solve(ad, MtlArray(Fb); dirichlet = :boundary)
