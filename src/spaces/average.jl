...
@inline function Mₕ₂(u::VectorElement) 
	...
	v = similar(u)
	backward_averagez!(v.values, u.values, npoints(mesh(u)))
...
Mₕ(u::VectorElement) = ntuple(i-> Mₕ(u, Val(i)), dim(u))

Mₕₓ(u::MatrixElement) = Elements(space(u), Mₕₓ(mesh(u)) * u.values)
Mₕᵧ(u::MatrixElement) = Elements(space(u), Mₕᵧ(mesh(u)) * u.values)
Mₕ₂(u::MatrixElement) = Elements(space(u), Mₕ₂(mesh(u)) * u.values)
Mₕ(u::MatrixElement) = (@assert dim(u) == 1; return Mₕₓ(u))
