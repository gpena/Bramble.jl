...
trial_space(l::LinearForm) = l.space

#function LinearForm(form_expr::F, S::SType) where {F<:Function, SType<:SpaceType}
    #vec = Element(S)
    #_, preallocated_f = preallocate(form_expr, vec)
    #return LinearForm(preallocated_f, S, S.backend)
#    return LinearForm(form_expr, S)
#end
...
Mass(s::SpaceType) = LinearMass{typeof(s), eltype(s)}(s, vec(FillArrays.Ones(eltype(s), ndofs(s))))

update!(m::LinearMass, v::AbstractFloat) = (fill!(m.vec, v))
update!(m::LinearMass, v::VectorElement) = (m.vec .= v)

assemble(m::LinearMass) = (m.vec .* m.space.innerh_weights.diag)::Vector{eltype(m.vec)}
