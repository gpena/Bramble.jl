...
Returns a [MatrixElement](@ref) implementing the backward finite difference matrix for the mesh grid of `Wₕ`, in the `x` direction. It is defined as being the (sparse) matrix representation of the linear operator defined by [`D₋ₓ(uₕ::VectorElement)`](@ref).
"""
...
Returns a [MatrixElement](@ref) implementing the backward finite difference matrix for the mesh grid of `Wₕ`, in the `y` direction. It is defined as being the (sparse) matrix representation of the linear operator defined by [`D₋ᵧ(uₕ::VectorElement)`](@ref).
"""
...
Returns a [MatrixElement](@ref) implementing the backward finite difference matrix for the mesh grid of `Wₕ`, in the `z` direction. It is defined as being the (sparse) matrix representation of the linear operator defined by [`D₋₂(uₕ::VectorElement)`](@ref).
"""
...
Returns a tuple of [MatrixElement](@ref)s implementing the backward finite difference operators in the `x`, `y`, and `z` directions. If the problem is 1D, it returns a single [MatrixElement](@ref).
"""
...
```math
\\textrm{D}_{-x} \\textrm{u}_h (x_i) = \\frac{\\textrm{u}_h(x_i) - \\textrm{u}_h(x_{i-1})}{h_i}
```

  - 2D and 3D case

```math
\\textrm{D}_{-x} \\textrm{u}_h (x_i, \\dots) = \\frac{\\textrm{u}_h(x_i, \\dots)-\\textrm{u}_h(x_{i-1}, \\dots)}{h_{x,i}}
```
"""
...
```math
\\textrm{D}_{-y} \\textrm{u}_h(x_i, y_j, \\dots) = \\frac{\\textrm{u}_h(x_i, y_j, \\dots)-\\textrm{u}_h(x_i, y_{j-1}, \\dots)}{h_{y,j}}
```
"""
...
```math
\\textrm{D}_{-z} \\textrm{u}_h(x_i, y_j, z_l) = \\frac{\\textrm{u}_h(x_i, y_j, z_l)-\\textrm{u}_h(x_i, y_j, z_)}{h_{z,l}}
```
"""
...
Returns a tuple of [VectorElement](@ref)s implementing the backward finite difference operators in the `x`, `y`, and `z` directions applied to `uₕ`. If the problem is 1D, it returns a single [VectorElement](@ref).
"""
...
Returns a [MatrixElement](@ref) resulting of the multiplication of the backward finite difference matrix [D₋ₓ(Wₕ::SpaceType)](@ref) with the [MatrixElement](@ref) `Uₕ`.
"""
...
Returns a [MatrixElement](@ref) resulting of the multiplication of the backward finite difference matrix [D₋ᵧ(Wₕ::SpaceType)](@ref) with the [MatrixElement](@ref) `Uₕ`.
"""
...
Returns a [MatrixElement](@ref) resulting of the multiplication of the backward finite difference matrix [D₋₂(Wₕ::SpaceType)](@ref) with the [MatrixElement](@ref) `Uₕ`.
"""
...
Returns a tuple of [MatrixElement](@ref)s implementing the backward finite difference operators in the `x`, `y`, and `z` directions applied to `Uₕ`. If the problem is 1D, it returns a single [MatrixElement](@ref).
"""
