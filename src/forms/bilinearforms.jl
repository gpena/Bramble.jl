...
@inline assemble!(A::SparseMatrixCSC, m::BilinearMass) = ( A.nzval .= m.vec .* innerh_weights(m.space1).diag )
...
function Diff(s1::SpaceType, s2::SpaceType)
    ...
    S = BilinearDiff{typeof(s1),typeof(s2), dim(s1), eltype(s1)}(s1, s2, Uh, Vh, E, falses(ndofs(s1)))
...
function update!(m::BilinearDiff{S1, S2, D, MType}, v::NTuple{D,T}) where {S1, S2, D, T, MType} 
    for i in 1:D
        ...
        x = innerplus_weights(m.space1, i).diag
...
mass(U::VecOrMatElem, V::VecOrMatElem; scaling::ScalingType = 1.0) = mass(U, V, Val(dim(U)), scaling)

function mass(U::VecOrMatElem,_,::Val{1}, scaling::ScalingType)
    D = _create_diagonal(test_space(U))
    #D = Diagonal(Vector{eltype(U)}(undef, ndofs(mesh(U))))
    @. D.diag = hmean_extended(mesh(U))
    _mat_scale!(D.diag, scaling)

    return D
end

stiffness(U::VecOrMatElem, V::VecOrMatElem; scaling::ScalingType = 1.0) = stiffness(U, V, Val(dim(U)), scaling)
...
advection(U::VecOrMatElem, V::VecOrMatElem; scaling::ScalingType = 1.0) = advection(U, V, Val(dim(U)), scaling)
