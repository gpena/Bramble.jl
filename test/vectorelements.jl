import Bramble: hmeanit, points, xmeanit, mesh
...
function __func2array_med(w::Array{T,D}, M::MType) where {MType,D,T}
    Xi = ntuple(i -> zip(hmeanit(M(i)), xmeanit(M(i)), Iterators.drop(xmeanit(M(i)), 1)), D)
...
function __init(::Val{D}) where {D}
    ...
    X = Domain(reduce(×, intervals))

    mesh = Mesh(X, dims, unif[D])
    Wh = GridSpace(mesh)
    u = Element(Wh, 1)
...
function vector_element_tests(::Val{D}) where {D}
    ...
    if D == 1
        @test validate_equal(z, sqrt(sum(hmeanit(mesh(Wh)))))
    ...
    Bramble._func2array!(w, __test_function, mesh(Wh))

    w2 = reshape(w, prod(dims))
    @test(validate_equal(u, w2))

    avgₕ!(u, __test_function)
    __func2array_med(w, mesh(Wh))
