...
function IdentityOperator(space::AbstractSpaceType)
...
struct OperatorAdd{D, LeftType <: LazyOp{D}, RightType <: LazyOp{D}} <: LazyOp{D}
    ...
    function OperatorAdd{D, LeftType, RightType}(
            left::LeftType, right::RightType) where {D, LeftType, RightType}
        new{D, LeftType, RightType}(left, right)
    end
end

function OperatorScale(scalar::S, op::LazyOp{D}) where {D, S}
    OperatorScale{D, S, typeof(op)}(scalar, op)
end
function GridFunctionScale(grid_function::V, op::LazyOp{D}) where {D, V}
    GridFunctionScale{D, V, typeof(op)}(grid_function, op)
end
function OperatorAdd(left::LazyOp{D}, right::LazyOp{D}) where {D}
...
@inline *(vh::AbstractVector, ops::NTuple{D, LazyOp{D}}) where {D} = ntuple(
    dim -> vh *
           ops[dim], Val(D))
@inline *(ops::NTuple{D, LazyOp{D}}, vh::AbstractVector) where {D} = ntuple(
    dim -> ops[dim] *
           vh, Val(D))
...
@inline innerₕ(uₕ::VectorElement, ::IdentityOperator) = innerh_weights(space(uₕ)) .*
                                                        uₕ.values
@inline innerₕ(uₕ::VectorElement, ::ZeroOperator) = zeros(eltype(uₕ), length(uₕ.values))
@inline innerₕ(uₕ::VectorElement,
    l::OperatorScale{D, S, OP}) where {
    D, S, OP <: Union{IdentityOperator, OperatorScale}} = innerh_weights(space(uₕ)) .*
                                                          l.scalar .* uₕ.values
@inline innerₕ(uₕ::VectorElement,
    l::GridFunctionScale{D, V, OP}) where {
    D, V, OP <: Union{IdentityOperator, OperatorScale}} = innerh_weights(space(uₕ)) .*
                                                          l.grid_function .* uₕ.values

# Modified inner plus
@inline inner₊(uₕ::VectorElement, ::IdentityOperator) = innerplus_weights(space(uₕ), Val(1)) .*
                                                        uₕ.values
@inline inner₊(uₕ::VectorElement, ::ZeroOperator) = zeros(eltype(uₕ), length(uₕ.values))
@inline inner₊(uₕ::VectorElement,
    l::OperatorScale{D, S, OP}) where {
    D, S, OP <: Union{IdentityOperator, OperatorScale}} = innerplus_weights(space(uₕ), Val(1)) .*
                                                          l.scalar .* uₕ.values
@inline inner₊(uₕ::VectorElement,
    l::GridFunctionScale{D, V, OP}) where {
    D, V, OP <: Union{IdentityOperator, OperatorScale}} = innerplus_weights(space(uₕ), Val(1)) .*
                                                          l.grid_function .* uₕ.values
...
function inner₊!(vₕ::AbstractVector, uₕ::NTuple{D, VectorElement}, l::NTuple{
        D, LazyOp{D}}) where {D}
    W = space(first(uₕ))
    for i in 1:D
        x = innerplus_weights(W, i)
...
@inline inner₊(l::NTuple{D, LazyOp{D}}, uₕ::Union{
    VectorElement, NTuple{D, VectorElement}}) where {D} = inner₊(uₕ, l)
