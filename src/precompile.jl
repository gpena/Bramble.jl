...
function _pc_indexed(Ωₕ::AbstractMeshType{1}, idx_tup)
	i = idx_tup[1]
	point(Ωₕ, i); spacing(Ωₕ, i); forward_spacing(Ωₕ, i)
	half_spacing(Ωₕ, i); half_point(Ωₕ, i); cell_measure(Ωₕ, i)
	return nothing
end

function _pc_indexed(Ωₕ::AbstractMeshType, idx_tup)
	point(Ωₕ, idx_tup); spacing(Ωₕ, idx_tup); forward_spacing(Ωₕ, idx_tup)
	half_spacing(Ωₕ, idx_tup); half_point(Ωₕ, idx_tup); cell_measure(Ωₕ, idx_tup)
	return nothing
end
...
function _pc_mesh_session(Ω, npts, unif, be, label::Symbol)
	...
	point(Ωₕ, idx); spacing(Ωₕ, idx); forward_spacing(Ωₕ, idx)

	dim(Ωₕ); dim(typeof(Ωₕ)); eltype(Ωₕ); eltype(typeof(Ωₕ))
	topo_dim(Ωₕ); set(Ωₕ); backend(Ωₕ); markers(Ωₕ)
	npoints(Ωₕ); npoints(Ωₕ, Tuple); size(Ωₕ); length(Ωₕ); axes(Ωₕ)
	points(Ωₕ); half_points(Ωₕ); half_spacings(Ωₕ)
	hₘₐₓ(Ωₕ); hₘᵢₙ(Ωₕ); is_uniform(Ωₕ); is_collapsed(Ωₕ(1))

	indices(Ωₕ); boundary_indices(Ωₕ); interior_indices(Ωₕ)
	is_boundary_index(Ωₕ, idx); index_in_marker(Ωₕ, label)
...
function _pc_linear_algebra(be)
	vector(be, 8); vector(be, 0)
	matrix(be, 4, 4); matrix(be, 0, 4); matrix(be, 4, 0)
	backend_eye(be, 4); backend_zeros(be, 4)
	backend_types(be); backend_types(typeof(be))
	vector_type(be); matrix_type(be); eltype(be); eltype(typeof(be))

	u = fill(1.0, 4); v = fill(2.0, 4); w = fill(0.5, 4)
	_dot(u, v, w)
...
function _pc_bramble_function(X, I_time, f, ft, pt)
	...
	if pt isa Number
		...
		bf(pt); bf(pt...); bf(collect(pt))
	end
	bft(0.5)(pt)
	bft(pt, 0.5)

	has_time(bf); has_time(typeof(bf)); has_time(bft)
	argstype(bf.wrapped); codomaintype(bf.wrapped)
	embed_function(X, bf)
	sprint(show, bf)
	return nothing
end
...
function _pc_geometry()
	...
	interval(I); point(0.5)
	cartesian_product(0.0, 1.0); cartesian_product(I)
	cartesian_product(((0.0, 1.0),)); cartesian_product(((0.0, 1.0), (2.0, 3.0)))
	box(0.0, 1.0); box((0.0, 0.0), (1.0, 1.0)); box((0.0, 0.0, 0.0), (1.0, 1.0, 1.0))

	for X in (I, R2, R3)
		dim(X); dim(typeof(X)); eltype(X); eltype(typeof(X))
		topo_dim(X); center(X); tails(X); set(X); point_type(X)
		get_boundary_symbols(X)
		for i in 1:dim(X)
			X(i); tails(X, i); projection(X, i)
		end
		sprint(show, X)
	end
	first(I); last(I); is_collapsed(I); is_collapsed(0.0, 1.0)

	0.5 in I; (0.5,) in I; [0.5] in I; SVector(0.5) in I
	(0.5, 1.0) in R2; [0.5, 1.0] in R2; SVector(0.5, 1.0) in R2
	return nothing
end
...
if PRECOMPILE_WORKLOAD
	@setup_workload begin
		...
		I = interval(0.0, 1.0)
		Ω1 = domain(I, :left => :left, :right => :right)

		S2 = I × interval(0.0, 2.0)
		...
		@compile_workload begin
			...
			stepsize(Ωₕ1); stepsize(Ωₕ2); stepsize(Ωₕ2, 1)
			locate_cell(Ωₕ1, 0.5); locate_cell(Ωₕ2, (0.5, 0.5))
			locate_cell(Ωₕ2, [0.5, 0.5])
			normal_vector(Ωₕ1, :left); normal_vector(Ωₕ2, :top)
			normal_vector(Ωₕ3, :front)

			# Function embedding, including the time-dependent path.
			_pc_bramble_function(I, I_time, x -> x + 1.0, (x, t) -> (x + 1.0) * t, 0.5)
			...
			for X in (I, S2)
				...
				d = domain(X, m)
				length(m); isempty(m)
				collect(labels(d)); collect(label_symbols(d))
				collect(label_conditions(d)); collect(marker_identifiers(d))
				center(d); tails(d); sprint(show, d); sprint(show, m)
			end
			collect(labels(domain(I, I_time, :moving => ((x, t) -> x > t))(0.5)))
		end
	end
end
