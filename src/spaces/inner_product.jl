...
inner₊(U::VecOrMatElem, V::VecOrMatElem) = (@assert dim(U) == 1 == dim(V); inner₊(U, V, Val(1)))# : @error "inner₊ not defined for 2D/3D functions"
...
function __snorm_aux(v, u, ::Val{D}) where D
	if D == 1
		_backward_differencex!(v.values, u.values, Base.Fix1(Bramble.hspace, mesh(u)(1)), npoints(u))
	end

	if D == 2
		_backward_differencey!(v.values, u.values, Base.Fix1(Bramble.hspace, mesh(u)(2)), npoints(u))
	end

	if D == 3
		_diff1!(v.values, u.values, Base.Fix1(Bramble.hspace, mesh(u)(3)), npoints(u))
	end
