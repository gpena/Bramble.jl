...
@inline _Eye(::Type{T}, npts::Int, ::Val{i}) where {T,i} = spdiagm(i => Ones(T, npts - abs(i)))
@inline _Eye(::Type{T}, npts::Int, ::Val{0}) where T = Eye(npts)

# shift functions
#@inline shift(Ωₕ::MeshType, ::Val{i}) where i = _Eye(eltype(Ωₕ), npoints(Ωₕ, Tuple)[1], Val(i))
#@inline shift(Ωₕ::MeshType, ::Val{0}) = Eye(npoints(Ωₕ, Tuple)[1])
...
@inline function _backward_finite_differencex!(out, in, hx::Vector, dims::NTuple{1,Int})
	N = dims[1]
	@assert length(out) == length(in) == N
	aux = in[1]
	out[1] = in[1] / hx[2]
